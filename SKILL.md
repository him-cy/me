...
## 触发后第一步：告知用户并列出问题清单（必做，勿直接执行）

用户说“爬取图片”时：

1. **先向用户说明已成功调用 Skill**，在对话中输出一行明确告知，例如：
   > 已成功调用 Skill「image-collect-pipeline」（目录名 `pexels-image-crawl`），下面会按 8 项参数与你确认后开始执行。

2. **然后输出以下问题清单收集信息，确认后再跑脚本**：
